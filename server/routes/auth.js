...
router.post('/request-code', async (req, res) => {
  ...
  const now = Date.now();
  db.saveVerifyCode({ email, code, expiresAt: now + VERIFY_CODE_TTL, createdAt: now });
...
router.post('/verify-code', (req, res) => {
  ...
  if (vc.code !== String(code)) {
    ...
    db.setVerifyFailures(email, fails);
  ...
  if (mode === 'register') {
    ...
    db.createUser({ id, email, passwordHash: hashPassword(String(password)), createdAt: now, updatedAt: now });
  ...
  db.createSession({ token, userId: user.id, deviceName: deviceName || null, createdAt: now, expiresAt: now + 30 * 24 * 3600 * 1000 });
...
router.post('/logout', (req, res) => {
  ...
  if (match) db.deleteSession(match[1].trim());
